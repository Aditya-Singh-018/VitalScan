...
export default function ResultsCard({ analysisResult }) {
  ...
  const getParameterStatus = (value, range) => {
    const parseRange = (rangeStr) => {
      ...
      return matches ? { min: parseFloat(matches[1]), max: parseFloat(matches[2]) } : null;
    };
  ...
  return <div className="max-w-4xl mx-auto space-y-6 py-8">
    ...
    >
                    <div className="flex items-center space-x-3">
                      {status === "low" && <AlertCircle className="w-5 h-5 text-blue-600" />}
                      {status === "high" && <AlertCircle className="w-5 h-5 text-red-600" />}
